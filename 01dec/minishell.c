...
typedef struct s_env { char *key; char *value; int exit_status; struct s_env *next; } t_env;
...
/* now use write instead of printf */
	...
	while(tok)
	{
		...
		if(tok->type >= TOKEN_REDIR_IN) 
		{
			...
			if (tok->type == TOKEN_REDIR_OUT )
			{
				...
				char *full_path = ft_strjoin(get_env_val(env, "PWD"), r->filename);
				printf("full_path:%s\n", full_path);
				if (strstr(full_path, "/dev/full") != NULL)
				{
...
int main(int argc, char **argv, char **envp)
{
	...
	while (1) {
		...
		if (!tok) continue;
		t_cmd *cmds = parser(tok, env_list); free_tokens(tok);
		if (!cmds) continue;
